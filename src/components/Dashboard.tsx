...
const METHODS = [
  ...
  { disc: 'フラクタル幾何学', name: 'Hurst指数 (R/S解析)',         desc: 'H>0.55=トレンド持続, H<0.45=平均回帰, それ以外=ランダム', impl: true },
...
export default function Dashboard() {
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-white">
      ...
      <header className="sticky top-0 z-10 bg-slate-900/95 backdrop-blur border-b border-slate-800 px-4 py-3">
        <div className="max-w-5xl mx-auto flex items-center justify-between gap-3">
          <div className="min-w-0">
            ...
            <p className="text-slate-500 text-[11px] sm:text-xs hidden sm:block">無駄なエントリーを極限まで減らすダッシュボード</p>
          </div>
          <div className="flex items-center gap-3 flex-shrink-0">
            ...
            <button
              ...
              disabled={refreshing}
              className="w-11 h-11 flex items-center justify-center rounded-xl bg-slate-700 hover:bg-slate-600 active:bg-slate-500 transition-colors text-slate-300 hover:text-white disabled:opacity-50 touch-manipulation"
              aria-label="手動更新"
            >
      ...
      <main className="max-w-5xl mx-auto px-4 pb-8 pt-4 space-y-4">
        {/* ── Entry Signal Banner ── */}
        {entrySignal && sigStyle && (
          <div className={`rounded-xl p-4 border ${sigStyle.bg}`}>
            <div className="flex items-start gap-3">
              <span className="text-xl mt-0.5 flex-shrink-0">{sigStyle.icon}</span>
              <div className="min-w-0">
                <div className={`text-base sm:text-lg font-bold ${sigStyle.color}`}>{entrySignal.label}</div>
                <div className="text-xs sm:text-sm text-slate-400 mt-0.5">{entrySignal.reason}</div>
              </div>
            </div>
          </div>
        )}

        {/* ── Timeframe Cards: horizontal scroll on mobile, grid on sm+ ── */}
        ...
        {/* ── 凡例 ── */}
        <div className="p-3 sm:p-4 bg-slate-800/50 rounded-xl border border-slate-700/50 text-xs text-slate-500">
          <p className="font-semibold text-slate-400 mb-2">凡例</p>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-1.5">
            <span>⚡ スクイーズ — BB幅が過去平均の75%未満</span>
            <span>💥 拡張中 — BB幅が過去平均の130%超</span>
